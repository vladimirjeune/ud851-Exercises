...
public class MainActivity extends AppCompatActivity {
    ...
    public void onClickOpenAddressButton(View v) {
        // TODOne (5) Store an address in a String
        String scheme = "geo";
        String address = "8408 Ravenswood Rd, New Carrollton, MD";
        String path = "0,0";


        // TODOne (6) Use Uri.parse with the appropriate scheme and query to form the Uri for the address #Used Uri.Builder
        ...
        // TODOne (7) Replace the Toast with a call to showMap, passing in the Uri from the previous step
        showMap(addressUri);
    }
    ...
    // TODOne (1) Create a method called showMap with a Uri as the single parameter
    // Do steps 2 - 4 within the showMap method
    public void showMap(Uri geoUri) {
        // TODOne (2) Create an Intent with action type, Intent.ACTION_VIEW
        Intent intent = new Intent(Intent.ACTION_VIEW);

        // TODOne (3) Set the data of the Intent to the Uri passed into this method
        intent.setData(geoUri);

        // TODOne (4) Verify that this Intent can be launched and then call startActivity
        if (intent.resolveActivity(getPackageManager()) != null ) {
            startActivity(intent);
        }
    }
