...
public class NotificationUtils {
    ...
    private static final int PENDING_INTENT_ACTION_DISMISS_ID = 4417;
    private static final int PENDING_INTENT_ACTION_INCREMENT_ID = 5417;
    ...
    public static NotificationCompat.Action ignoreReminderAction(Context context) {
        ...
        Intent wrisIntent = new Intent(context, WaterReminderIntentService.class);

        //      TODO (7) Set the action of the intent to designate you want to dismiss the notification
        wrisIntent.setAction(ReminderTasks.ACTION_DISMISS_NOTIFICATION);

        //      TODO (8) Create a PendingIntent from the intent to launch WaterReminderIntentService
        PendingIntent pendingIntentDismiss = PendingIntent.getService(
                context,
                PENDING_INTENT_ACTION_DISMISS_ID,
                wrisIntent,
                PendingIntent.FLAG_UPDATE_CURRENT
        );

        //      TODO (9) Create an Action for the user to ignore the notification (and dismiss it)
        NotificationCompat.Action dismissReminderAction = new NotificationCompat.Action(
                R.drawable.ic_cancel_black_24px,
                "No Thanks!",
                pendingIntentDismiss
        );

        //      TODO (10) Return the action
        return dismissReminderAction;
    }
    ...
    public static NotificationCompat.Action drinkWaterAction(Context context) {
        ...
        wrisIntent.setAction(ReminderTasks.ACTION_INCREMENT_WATER_COUNT);
        ...
        PendingIntent incrementWaterIntent = PendingIntent.getService(
                context,
                PENDING_INTENT_ACTION_INCREMENT_ID,
                wrisIntent,
                PendingIntent.FLAG_UPDATE_CURRENT
        );
